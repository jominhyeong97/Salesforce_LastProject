...
8. [기타 기능 3건](#8-기타-기능-3건)
9. [회고](#9-회고)
10. [기술 스택 & 메타데이터 규모](#10-기술-스택--메타데이터-규모)
11. [디렉토리 구조](#11-디렉토리-구조)
12. [빌드 / 배포](#12-빌드--배포)

> 이 README는 발표 슬라이드(29페이지)와 동일한 순서로 구성되어 있습니다. 슬라이드 원본은 저장소 [`index.html`](index.html)을 브라우저로 열면 그대로 재생됩니다.
...
각 기능의 핵심 장면은 아래 [주요 기능 5건](#7-주요-기능-5건) 섹션에 **GIF로 임베드**되어 README에서 클릭 없이 바로 재생됩니다. [기타 기능](#8-기타-기능-3건) 시연은 Google Drive 영상으로 제공합니다.

> 일부 GIF(견적 라인·재견적)는 분량 조절을 위해 약간 빠르게 재생됩니다. 단계별 원본 영상(MP4)은 [`docs/videos/`](docs/videos/)에 포함되어 있습니다. 발표 슬라이드 29페이지 원본은 [`index.html`](index.html)을 브라우저로 열어 확인할 수 있습니다.
...
| **P4** | 장기 미수금 식별·분할 입금 추적을 영업이 직접 챙겨야 함 | 미수금 카드 + 입금 등록 (기능 1 · 기타 기능 2) |
...
주요 기능 외에 L2C 흐름을 보강하는 3가지 기능입니다.

### 기타 기능 1 · 리드 메모 AI 자동 채움

전화 · 방문 · RFQ 메일 **3채널로 인입된 Lead**에 대해, AE가 한 줄로 적은 통화·방문 메모를 AI가 분석해 **고객사 · 관심 품목 · 요구 사항 · 다음 액션** 4개 필드로 자동 구조화합니다.

- 인입 채널 3종 · AI 메모 자동 구조화 · 자동 필드 4개
- 기술: Flow · Prompt Builder
- 시연: [▶ 리드 메모 AI 자동 채움 (Google Drive)](https://drive.google.com/file/d/1x8EO2UX7MMcAvXOiXHcpu4J40PkgqmQL/view)

### 기타 기능 2 · 장기 미수금 알림

**60일 이상 미수 주문을 자동 식별**해 해당 고객사 상단에 빨간 띠를 표시합니다. 후속 조치(분할 입금·완납)가 등록되면 자동 해제되며, 일배치 스캔으로 운영 부담 없이 동작합니다.

- 60일+ 자동 식별 · daily 스캔 · 조치 후 자동 해제
- 기술: Flow · Apex
- 시연: [▶ 장기 미수금 알림 (Google Drive)](https://drive.google.com/file/d/14NU6BEqdotfrZS0lv6Ul6c38Aq6y2dkp/view)

### 기타 기능 3 · 기회 인사이트 분석

정량 점수는 **Apex 결정적 계산**(단계 50 · 활동 20 · Account 15 · Engagement 15)으로 4개 축 분해하고, 자연어 요약과 다음 액션 2~3개는 **Prompt Builder(GPT-4.1)**가 생성합니다. **LLM 장애 시 Apex fallback**으로 graceful degrade — 점수와 액션은 항상 노출됩니다.

- 100점 만점 · 서브 점수 4축 분해 · AI + Fallback graceful degrade
- 기술: Apex · Prompt Builder · LWC
- 화면: [기회 인사이트 카드 (Google Drive)](https://drive.google.com/file/d/1L1UDJw4K2ohip381TmdhvilYJdZcK5Lu/view)
...
index.html          # 발표 슬라이드 29페이지 (브라우저로 열기)
